...
def verify_user_status_from_user_list(logged_in_admin: Page, username: str) -> None:
    ...
    expect(page.get_by_role("link", name=f"{username}")).to_be_visible()


@then(parsers.parse("then user tries to login in with valid credentials {username} {password} ask for update password in login page"))
def verify_user_login(logged_in_admin: Page, username: str, password: str) -> None:
    ...
    expect(client_account_page.get_by_text("You need to change your")).to_be_visible()
